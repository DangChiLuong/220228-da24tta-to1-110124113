...
void Tim_Vi_Tri_Co_GTLN_Dau_Tien (float A[], int n);
void In_Vi_Tri_GTLN (float A[], int n);
...
int main()
{
	...
	//Tim vi tri co GTLN
	Tim_Vi_Tri_Co_GTLN_Dau_Tien(A,n);
	//In GTLN
	In_Vi_Tri_GTLN(A,n);
...
void Tim_Vi_Tri_Co_GTLN_Dau_Tien (float A[], int n) 
{
	printf ("\n\nVi tri GTLN dau tien la: ");
	for (int i = 0 ; i < n ; i++)
	{
		if(A[i] == Tim_GTLN_Trong_Mang(A,n))
		{
			printf("%d\t", i);
			break;
		}
